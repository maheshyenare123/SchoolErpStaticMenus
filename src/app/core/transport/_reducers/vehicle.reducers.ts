...
export function vehiclesReducer(state = initialVehiclesState, action: VehicleActions): VehiclesState {
  switch (action.type) {
    ...
    case VehicleActionTypes.VehicleOnServerCreated:
      return {
        ...state
      };
    case VehicleActionTypes.VehicleCreated:
      return adapter.addOne(action.payload.vehicle, {
        ...state, lastCreatedVehicleId: action.payload.vehicle.id
      });
    case VehicleActionTypes.VehicleUpdated:
      return adapter.updateOne(action.payload.partialVehicle, state);
    ...
    // }
    case VehicleActionTypes.OneVehicleDeleted:
      return adapter.removeOne(action.payload.id, state);
    case VehicleActionTypes.ManyVehiclesDeleted:
