...
import { StudentFeeDepositesDataSource, StudentFeeDepositeModel, StudentFeeDepositesPageRequested, OneStudentFeeDepositeDeleted, ManyStudentFeeDepositesDeleted,StudentFeeDepositeService } from 'src/app/core/fees-collection';
...
// import { StudentFeeDepositeProfileViewDialogComponent } from '../studentfeedeposite-profile-view/studentfeedeposite-profile-view.dialog.component';

@Component({
  selector: 'kt-search-fees-payment',
  templateUrl: './search-fees-payment.component.html',
  styleUrls: ['./search-fees-payment.component.scss']
})
export class SearchFeesPaymentComponent implements OnInit {

 // Table fields
dataSource: StudentFeeDepositesDataSource;
displayedColumns = ['paymentId', 'date', 'name', 'class', 'feesGroup', 'feesType', 'mode','amount','discount','fine', 'actions'];
...
/**
    ...
    const _saveMessage = studentfeedeposite.feeGroupId > 0 ? 'Edit product' : 'Create product';
  ...
  onStudentFeeDepositeProfileView(studentfeedeposite: StudentFeeDepositeModel) {
		let saveMessageTranslateParam = 'ECOMMERCE.CUSTOMERS.EDIT.';
    const _saveMessage = studentfeedeposite.feeGroupId > 0 ? 'StudentFeeDeposite Profile View' : 'Create product';
    
		const _messageType = studentfeedeposite.feeGroupId > 0 ? MessageType.Update : MessageType.Create;
		// const dialogRef = this.dialog.open(StudentFeeDepositeProfileViewDialogComponent, { data: { studentfeedeposite } });
		// dialogRef.afterClosed().subscribe(res => {
		// 	if (!res) {
		// 		return;
		// 	}
  ...
  onStudentFeeDepositeFeesCollect(studentfeedeposite: StudentFeeDepositeModel) {
		let saveMessageTranslateParam = 'ECOMMERCE.CUSTOMERS.EDIT.';
    // const _saveMessage = studentfeedeposite.id > 0 ? 'StudentFeeDeposite Fees Collect' : 'Create product';
    
		// const _messageType = studentfeedeposite.id > 0 ? MessageType.Update : MessageType.Create;
		// const dialogRef = this.dialog.open(StudentFeeDepositeFeesCollectDialogComponent, { data: { studentfeedeposite } });
		// dialogRef.afterClosed().subscribe(res => {
		// 	if (!res) {
		// 		return;
		// 	}
