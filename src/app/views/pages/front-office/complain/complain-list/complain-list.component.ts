...
import { ComplainsDataSource, ComplaintModel, ComplaintsPageRequested, OneComplaintDeleted, ManyComplaintsDeleted } from '../../../../../core/front-office';
...
displayedColumns = ['id','complaintType','name', 'phone', 'date', 'actions'];
 ...
 **/
 ...
 // Init DataSource
 this.dataSource = new ComplainsDataSource(this.store);
...
getAllComplainList() {
  ...
  //   this.complainsResult = content.map((key) => ({ ...key }));
...
loadComplainsList() {
  ...
  this.store.dispatch(new ComplaintsPageRequested({ page: queryParams }));
  // get api 
  this.selection.clear();
}
 ...
 */
deleteProducts() {
  const _title = 'Complain s Delete';
  const _description = 'Are you sure to permanently delete selected Complains?';
  const _waitDesciption = 'Complain s are deleting...';
  const _deleteMessage = 'Selected Complains have been deleted';
  ...
  dialogRef.afterClosed().subscribe(res => {
    ...
    this.store.dispatch(new ManyComplaintsDeleted({ ids: idsForDeletion }));
    this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
    this.selection.clear();
  });
}
