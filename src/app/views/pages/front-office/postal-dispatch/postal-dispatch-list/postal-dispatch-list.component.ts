...
import { PostalDispatchsDataSource, DispatchReceiveModel, OnePostalDispatchDeleted, ManyPostalDispatchsDeleted, PostalDispatchsPageRequested } from '../../../../../core/front-office';
...
import { PostalDispatchEditDialogComponent } from '../postal-dispatch-edit/postal-dispatch-edit.dialog.component';

@Component({
  selector: 'kt-postal-dispatch-list',
  templateUrl: './postal-dispatch-list.component.html',
  styleUrls: ['./postal-dispatch-list.component.scss']
})
export class PostalDispatchListComponent implements OnInit {

 // Table fields
dataSource: PostalDispatchsDataSource;
...
postalDispatchsResult: DispatchReceiveModel[] = [];
 ...
 **/
 const paginatorSubscriptions = merge(this.sort.sortChange, this.paginator.page).pipe(
   tap(() => this.loadPostalDispatchsList())
 )
 .subscribe();
 ...
 // Init DataSource
 this.dataSource = new PostalDispatchsDataSource(this.store);
...
loadPostalDispatchsList() {
  ...
  this.store.dispatch(new PostalDispatchsPageRequested({ page: queryParams }));
  // get api 
  this.selection.clear();
}
...
//delete api call
    this.store.dispatch(new OnePostalDispatchDeleted({ id: _item.id }));
...
/**
	...
	addPostalDispatch() {
		...
		this.editPostalDispatch(newCustomer);
	}
...
isAllSelected() {
  ...
  const numRows = this.postalDispatchsResult.length;
...
masterToggle() {
  if (this.isAllSelected()) {
    ...
    this.postalDispatchsResult.forEach(row => this.selection.select(row));
  }
}
