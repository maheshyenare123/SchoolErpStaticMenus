...
import { VisitorBooksDataSource, VisitorBookModel, VisitorBooksPageRequested, OneVisitorBookDeleted, ManyVisitorBooksDeleted } from '../../../../../core/front-office';
...
import { VisitorBookEditDialogComponent } from '../visitor-book-edit/visitor-book-edit.dialog.component';

@Component({
  selector: 'kt-visitor-book-list',
  templateUrl: './visitor-book-list.component.html',
  styleUrls: ['./visitor-book-list.component.scss']
})
export class VisitorBookListComponent implements OnInit {
// Table fields
dataSource: VisitorBooksDataSource;
displayedColumns = ['purpose','name', 'phone', 'date', 'inTime', 'outTime', 'actions'];
...
// Selection
selection = new SelectionModel<VisitorBookModel>(true, []);
visitorsBookResult: VisitorBookModel[] = [];
 ...
 **/
 const paginatorSubscriptions = merge(this.sort.sortChange, this.paginator.page).pipe(
   tap(() => this.loadAddmissionVisitorsBookList())
 )
 .subscribe();
 ...
 const searchSubscription = fromEvent(this.searchInput.nativeElement, 'keyup').pipe(
   ...
   tap(() => {
     ...
     this.loadAddmissionVisitorsBookList();
   })
 )
 .subscribe();
 ...
 // Init DataSource
 this.dataSource = new VisitorBooksDataSource(this.store);
 ...
 * Load Products List
 */
loadAddmissionVisitorsBookList() {
  ...
  this.store.dispatch(new VisitorBooksPageRequested({ page: queryParams }));
  // get api 
  this.selection.clear();
}
...
//delete api call
    this.store.dispatch(new OneVisitorBookDeleted({ id: _item.id }));
...
deleteProducts() {
  ...
  dialogRef.afterClosed().subscribe(res => {
    ...
    this.store.dispatch(new ManyVisitorBooksDeleted({ ids: idsForDeletion }));
    this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
    this.selection.clear();
  });
}
...
/**
	 ...
	 */
	addVisitorBook() {
		const newCustomer = new VisitorBookModel();
		newCustomer.clear(); // Set all defaults fields
		this.editVisitorsBook(newCustomer);
	}
...
isAllSelected() {
  ...
  const numRows = this.visitorsBookResult.length;
...
masterToggle() {
  if (this.isAllSelected()) {
    ...
    this.visitorsBookResult.forEach(row => this.selection.select(row));
  }
}
