...
import { ItemIssueEditDialogComponent} from '../item-issue-edit/item-issue-edit.dialog.component';

@Component({
  selector: 'kt-item-issue-list',
  templateUrl: './item-issue-list.component.html',
  styleUrls: ['./item-issue-list.component.scss']
})
export class ItemIssueListComponent implements OnInit {

 // Table fields
dataSource: ItemIssuesDataSource;

displayedColumns = ['itemName', 'itemCategory', 'issueReturn', 'issueTo', 'issueBy', 'quantity', 'isReturned','actions'];
 ...
 *
 * @param _item: ItemIssueModel
 */
deleteItemIssue(_item: ItemIssueModel) {
...
//delete api call
    this.store.dispatch(new OneItemIssueDeleted({ id: _item.id }));
...
deleteProducts() {
  ...
  dialogRef.afterClosed().subscribe(res => {
    ...
    this.store.dispatch(new ManyItemIssuesDeleted({ ids: idsForDeletion }));
    this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
    this.selection.clear();
  });
}
...
/**
	 ...
	 */
	addItemIssue() {
		const newCustomer = new ItemIssueModel();
		newCustomer.clear(); // Set all defaults fields
		this.editItemIssue(newCustomer);
	}
	 ...
	 */
	editItemIssue(itemIssue: ItemIssueModel) {
		let saveMessageTranslateParam = 'ECOMMERCE.CUSTOMERS.EDIT.';
    const _saveMessage = itemIssue.id > 0 ? 'Edit Postal Receive' : 'Create Postal Receive';
    
		const _messageType = itemIssue.id > 0 ? MessageType.Update : MessageType.Create;
		const dialogRef = this.dialog.open(ItemIssueEditDialogComponent, { data: { itemIssue } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
...
isAllSelected() {
  ...
  const numRows = this.itemIssuesResult.length;
...
masterToggle() {
  if (this.isAllSelected()) {
    ...
    this.itemIssuesResult.forEach(row => this.selection.select(row));
  }
}
